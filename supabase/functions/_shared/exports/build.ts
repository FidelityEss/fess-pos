...
function aboutRows(claim: Claim, visits: number, prepared: Date): Table['rows'] {
  ...
  return [
    ...
    ['Asked for', timeCell(claim.export.created_at, tz)],
    ['Prepared', timeCell(prepared.toISOString(), tz)],
    ['Times', `Shown in ${tz} time. Excel keeps no time zone; the CSV and the bank API also give the offset.`],
