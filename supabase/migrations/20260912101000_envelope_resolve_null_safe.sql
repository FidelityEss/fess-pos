-- Fix (pgTAP on fess-pos-qa, 2026-09-12): the "held" test in admin_envelope_resolve evaluated to NULL for a received
-- envelope with no waiting_on, so `if not (...)` did not refuse it. A plain received envelope (landed, not yet
-- processed) must not be resolvable; only rejected / conflict / deferred / held (waiting_on.reprocess) ones are.
...
begin
  ...
  if not (v.state in ('rejected', 'conflict', 'deferred') or (v.state = 'received' and coalesce(v.waiting_on ? 'reprocess', false))) then
