...
| `packages/fess_pos/` | Flutter module: the public contract, encrypted local store, POS API client and sign-in, outbox, pull and sync engines, the agent's jobs drawn from the server's views, reason forms and job actions (accept / can't take / unable), the map with directions, authorisation cards with the server's QR, push hints and deep links, and the walking-skeleton inspection (begin, camera photo, signature, declaration, submit, evidence upload). `HOST_INTEGRATION.md` lists the wiring a host adds; `example/` is the harness host, which signs in to QA |
| `packages/fess_pos_engine/` | Pure-Dart engine: JCS hashing, the rules evaluator, and the form resolver and validator for the reason-form components; the other components arrive with T3-01. Runs the same fixtures as the TS engine |
...
| `tools/` | `scenarios/` (scenario seeder, QA smoke test, `module-harness.ts` for a QA sign-in in the module's harness, `skeleton.ts` for the walking-skeleton inspection's QA test bank and job), `postman/` (collection for the module API, QA test users), `ops/` (pgTAP on QA, QA-link check, remote purge), `seed/` (definition seed builder), `vendor-engine.mjs` |
