...
| `packages/fess_pos/` | Flutter module: the public contract, encrypted local store, POS API client and sign-in, outbox, pull and sync engines, the agent's jobs drawn from the server's views, reason forms and job actions (accept / can't take / unable), the map with directions, authorisation cards with the server's QR, and push hints and deep links. `HOST_INTEGRATION.md` lists the wiring a host adds; `example/` is the harness host, which signs in to QA |
